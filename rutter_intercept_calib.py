...
def rutterIntercept(time,calib_vars,R,T,u,rh,Rn):
    # --- Canopy Parameters ---
    p = 0.25         # free throughfall [-]
    S = calib_vars[0]          # canopy storage capacity [m] (typically 0.5–2 mm)
    # b = 3.7          # first drainage exponential coefficient [?]
    # a = 10          # second drainage exponential coefficient [?]
    b = calib_vars[1] # first drainage exponential coefficient [?]
    a = calib_vars[2]          # second drainage exponential coefficient [?]
